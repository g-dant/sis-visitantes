...
def listar_visitantes(sessao = Depends(usuario_logado)):
  return VisitanteService.listar()


@router.get("/visitantes/{id_visitante}")
def buscar_visitante(id_visitante: int, sessao = Depends(usuario_logado)):
  return VisitanteService.buscar_por_id(id_visitante)


@router.post("/visitantes")
def criar_visitante(
  request: VisitanteRequest,
